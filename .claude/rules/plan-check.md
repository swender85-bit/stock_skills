...
- **手取り（税引後）での比較**: 売却を伴う案は `switching_hurdle` を必ず計算する
  （含み益があると乗り換えは 20.315% のハンデを背負う。土曜設計書 提案3）
- **入金で代替できるか**: 蓄積期のリバランスは売買ではなく入金で行うのが原則。
  `runway.funding_options()` で (a)売却 /(b)現金 /(c)入金待ち /(d)規模縮小 を比較する
  （土曜設計書 提案9）
- **注意予算**: 銘柄数を増やす案は、週次レビュー時間で維持できるかを確認する
...
- **因子双子リスク（土曜設計書 提案2）**: **その買い候補は既存保有の因子双子ではないか。**
  業種コードが違っても値動きが同一なら、それは実質的な買い増しであり分散は改善しない。
  セクターHHIはこれを分散とみなすので、HHIが下がることを根拠にしてはならない。
  `src.core.screening.marginal_bridge.build_marginal_view()` で限界スコアと
  因子双子を確認する
- **通貨の二重ロング（土曜設計書 提案2）**: 米国株と日本の輸出企業を同時に持つと、
  「地域は分散、為替は集中」になる。**円高シナリオでPFの大半が同時に不利になる**。
  セクター・地域HHIはこれを全く検出しない。`exposure.describe_tilt()` で確認する
- **平時相関の過信**: 相関は暴落時に1へ収束する。「相関0.11だから分散効果あり」は
  平時の話であり、ストレス時の相関と併記されていなければ根拠にならない
...
**判断原則**: 「ユーザーがこの出力を見て、迷わず行動できるか？」を基準に判定する。チェック項目はガイドラインであり、項目にない問題も自律的に検出する。
...
#### Tax Checker（土曜設計書 提案3/9）

**判断原則**: 「この提案は**税引後**でも改善するか？」を基準に判定する。
税引前の比較しかしていない提案は、含み益がある限り必ず 20.315% のハンデを隠している。

必須チェック:
- 売却を伴う全提案に `switching_hurdle`（乗り換え損益分岐率）が添付されているか
  - `python3 -c "from src.core.portfolio.tax import switching_hurdle; ..."` または
    `what-if --remove` / `adjust` の出力（両者は自動で添付する）
- 期待優位 < 損益分岐 の乗り換えは **FAIL（自動却下）**
- 資金収支の判断に**税引後**の差額を使っているか（税引前で「買える」と読むと実際には足りない）
- **入金代替案が併記されているか**。売却せず N週後の入金で同じエクスポージャーが得られる場合、
  売却案を推奨してはならない（`src.core.portfolio.runway.funding_options`）
- NISA 口座の損失は損益通算できない。NISA 保有の損切りに税務上の救済を期待していないか
- 節税を**理由**にした売却提案になっていないか（税は却下する側にのみ使う）

判定: PASS / FAIL（税引後で改善しない、または入金代替案が未検討）

#### Risk Checker

**判断原則**: 「この提案を実行して、想定外の損失が発生しないか？」を基準に判定する。リスク項目はガイドラインであり、項目にないリスクも自律的に検出する。
