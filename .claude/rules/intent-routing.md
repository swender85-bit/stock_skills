...
| **政策** | 事前に決めた方針を引く・決める | 政策、方針、決めてた、撤退条件、どうするんだっけ、急変時の売買相談 |
| **メタ** | システム自体について知りたい | 何ができる、機能一覧、改善、カイゼン |

> ⚠️ **急変時の最優先ルール（Fable5 案A）**: 下落・急変を含む売買相談（「-3%だけど売るべき？」「暴落したけどどうする？」）は、
> **分析より先に政策ドメインを引く**。詳細は `.claude/rules/policy-ledger.md`。
...
### 分析ドメイン → 個別銘柄は「常に全力」（deep pack 経由）

> **最優先ルール**: 個別銘柄について聞かれたら、軽い聞き方（「これどう？」「持ってて平気？」）でも
> **毎回ブリーフィングパックを作って深掘りする**。数字を並べるだけの薄い回答はしない。

```bash
python scripts/build_briefing_pack.py --symbol <SYMBOL>
```

→ 出力された `output/briefing/<SYMBOL>_YYYYMMDD.json` を読み、
**`.claude/prompts/stock_deep.md` の執筆仕様に従って synthesis する**。

パックには 価格・ファンダ・テクニカル・前回差分・競合/ベルウェザー・指数・ニュース・
今後の日程（決算/配当/FOMC）・過去のテーゼ/懸念/lesson が入っている。
材料集めは Python（トークン0）が済ませてあるので、Claude は解釈と処方に専念する。

**個別パックには週次と同じ判断層が全部入っている**（2026-08-06 に配線）:

| フィールド | 内容 |
|:---|:---|
| `policy` | 政策上の応答。**トリガー成立時は新しく判断せず既定の応答を出す** |
| `falsification` | 反証条件の点検。価格より先に信念の変化を見る |
| `primary_filings` | 開示原文（**深度0の一次観測**）。ニュースより系譜が強い |
| `external_views` | 外部批評家。`citation` に従い、未測定なら根拠にしない |
| `assumption_space` | 前提の衝突。計画の実行条件が揃うとこの銘柄が毀損する構造 |
| `forward` | 前方イベント・政策カバレッジの穴・トリガー距離 |
| `projection` | 1ヶ月/3ヶ月/6ヶ月/3年のレンジ（現在値=100の指数）。**予測ではなく前提σの帰結**。前提σと実測σの乖離（`calibration`）つき |

🔴 **「これから」を書かずに個別分析を終えてはならない。** 短期(1ヶ月)/中期(3ヶ月)/
長期(半年以降)で、①レンジ ②何がそれを決めるか ③外れる条件 の3点を書く
（`.claude/prompts/stock_deep.md` の 7.5）。**確率は書かない。**
3つの時間軸で結論が同じなら、それは分析していない。

⚠️ **判断の質を質問の形式で変えないこと。** 「週次だから丁寧、個別だから雑」は禁止。
以前は政策・反証条件・一次観測が PF パックにしか無く、
**「〇〇売るべき？」という最も政策が要る場面で政策が引かれていなかった。**

なお **買い推奨の禁止は週次（土曜）の制約であって、個別質問には課さない。**
「トヨタ買うべき？」に答えないのは機能ではなく欠陥。

- 対象: 「〇〇ってどう？」「〇〇買うべき？」「〇〇持ってて大丈夫？」「〇〇を調べて」
  「〇〇を深掘り」等、**特定銘柄への評価・判断を求める発言すべて**。
- 例外（パック不要）: 単なる株価照会（「今いくら？」）、売買記録（「買った」）、
  ウォッチ追加のみ、過去データ照会（`/graph-query`）。
- `--no-moomoo` / `--no-context` で外部依存を切れる。どれが落ちてもパックは必ず返る。
- パックの `meta.warnings` や空フィールドは「材料なし」ではなく**「取得できなかった」**として扱う。

**保有全体（週次相当）を深掘りしたいとき**（「週次レポート出して」「週次やって」
「PF全体を深く見て」「今週のPFまとめて」「土曜の待たずに今見たい」）:

```bash
python scripts/build_briefing_pack.py          # 保有全体パック
```
...
### 保有構成の取り込み → `scripts/import_rakuten_csv.py`

ユーザーが売買して保有構成が変わったときだけ実行する。**株価は毎回自動取得されるので、
価格のためにCSVを落とす必要はない。**

```
「CSV置いたよ」「保有更新して」「楽天のCSV取り込んで」 → python scripts/import_rakuten_csv.py
「保有変わった」「買ったから反映して」                → 同上（Downloads の最新を自動検出）
「何が変わった？」（取り込み前）                      → --dry-run で差分だけ表示
```

- 入力: 楽天証券 → マイメニュー → 保有商品一覧（すべて） → CSVで保存
- `assetbalance*.csv` を Downloads から自動検出する。パス指定は `--path`
- 国内株・米国株・投信・外貨預り金・為替が**すべて1ファイルに入る**
- `leverage` / `unit_divisor` / `note` は既存 YAML から引き継がれる
...
### 答え合わせドメイン → `scripts/track_outcomes.py`（学習ループ / upgrade v1.0 Phase 4）

過去に Screen で挙げた・Report で判断した銘柄が、その後どうなったかを現在株価で答え合わせし、命中率を集計する。

```
「答え合わせして」「判断当たってた？」        → python scripts/track_outcomes.py
「スクリーニングの成績は？」「命中率は？」     → python scripts/track_outcomes.py
「過去のおすすめは当たった？」               → python scripts/track_outcomes.py
「どのpresetが当たってる？」                → python scripts/track_outcomes.py（preset別命中率）
「外した銘柄から学びたい」                   → python scripts/track_outcomes.py（大幅下落を lesson自動記録）
```

- 出力: `docs/OUTCOMES.md`（総合命中率・30/90/180日バケット・preset別・学ぶべき外し）
- 大きく外した買い判断（既定 -20%以上・90日以上経過）は lesson ノートへ自動記録（重複はスキップ）
...
### 一次情報ドメイン → fetch_filings（開示原文）

**「なぜ動いたか」を調べるときの最優先経路。** ニュースやアナリスト見解より系譜が強い。

```
「なんで上がってるの？」「急騰の理由は？」「材料は？」  → --symbol <SYM> --explain-move（日本株）
「決算資料見せて」「10-K」「開示」「有報」「適時開示」   → --symbol <SYM>
「誰が買ってるの？」「大量保有」「5%ルール」            → --symbol <SYM> --explain-move
「本当の売上は？」「開示の数字で」「XBRL」             → --symbol <SYM> --financials
「一次情報つながってる？」                          → --status
```

**回答時の絶対ルール**:
- 取得できなかったとき「開示が無かった」「材料なし」と書かない。**「取得できなかった」**
- 大量保有報告書が**無いこと**を「需給要因ではない」の証明にしない
  （5営業日以内の提出なので急騰と同時には出ない。TDnet の適時開示は EDINET に無い）
- ETF に決算は**存在しない**。「取得できなかった」と書かない
- 再配信サービス経由の開示を**一次観測として扱わない**

**判定キーワード**: 「なんで上がった」「急騰の理由」「材料」「開示」「決算資料」
「10-K」「10-Q」「8-K」「有報」「臨時報告書」「大量保有」「XBRL」「一次情報」
...
### 読書ドメイン → `scripts/ingest_reading.py`（読書台帳）

**何かを読んだら、その場で30秒で記録する。** 詳細は `.claude/rules/reading-ledger.md`。

```
「これ読んだ」「この記事入れといて」「取り込んで」    → ingest（本文は WebFetch で取ってから stdin で渡す）
「メモ: 〜」「読んだ感想」「気づいたこと」            → --own（自分の考え）
「何読んだっけ」「取り込み状況」                      → --status
「偏ってる？」「何に偏ってる？」「読書の偏り」        → --audit
「批判的な材料も読んでる？」                          → --audit（批判ゼロの保有）
「情報源かたよってない？」「同じ人ばかり読んでる？」  → --audit（情報源HHI）
```

**絶対に守ること**:

- 🔴 **`ingested_at`（知った時刻）は遡れない。** 「後でまとめて」を勧めない
- 🔴 取り込みが0件のとき「読んでいない」ではなく**「記録が始まっていない」**と言う
- ユーザーが URL を貼ったら、まず WebFetch で本文を取り、`--stdin` で渡す。
  CLI 自身は Web を取得しない（取り込み経路にネットワークと Bash を同時に持たせない）
- 保有しているが thesis の無い銘柄に触れたら、取り込み後に thesis 草稿を促す。
  **書く動機が発生するのは、読んだ直後だけである**

### 概念ドメイン → `src/core/reading/concepts.py`

```
「これ概念にしておいて」「一般化して」「他の銘柄にも使える話」→ 概念として登録
「〇〇って概念ある？」「前に整理した考え方」                  → 既存概念を検索
```

- 🔴 **反証事例を書けないものは概念にしない。** thesis に置く
- 「QCOMは割安」は概念ではない（銘柄依存）。「レバレッジETFの逓減」は概念
- 作成前に必ず既存を検索する（同じことを別名で作るのが最大の失敗モード）

### 政策ドメイン → `scripts/manage_policy.py` (Fable5 案A)

**判断と執行の時間分離**。ストレス下の即興判断を構造的に排除するため、
急変時の売買相談は「新しく分析する」前に「平時に決めた政策を引く」。

```
「日経-3%。トヨタ売るべき？」   → ask（政策照会が先。分析は政策が無いときだけ）
「暴落したけどどうする？」      → ask（保有各銘柄の政策を引く）
「撤退条件どうしてたっけ？」    → ask / list
...
1. **急変系の質問では、まず `ask` を実行して政策上の応答を提示する。**
   分析（stock-report / market-research）はその後、またはユーザーが明示的に求めた場合のみ。
2. 政策が無い銘柄は従来どおり分析してよい（非破壊）。ただし回答末尾で政策の登録を促す。
3. トリガー成立・接近中に「やっぱり売りたい/買いたい」と言われたら、
   **冷却期間と逸脱記録を明示してから**ユーザーの判断を仰ぐ。勝手に政策を書き換えない。
4. 登録時、曖昧な条件（「なんか下がったら」）は測定可能な形に言い換えてもらう。
   使える指標は `MEASURABLE_METRICS`（drawdown_pct / rsi / per / operating_cf 等）。
5. **失効期限は必ず聞く。** 無期限政策は登録できない（硬直化の抑制策）。
...
📰 深掘りリサーチ
  ...
  → 分析時は保有銘柄＋主要指数のニュースを自動監視（Finnhub + yahoo）
