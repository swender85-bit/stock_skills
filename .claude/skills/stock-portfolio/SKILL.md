...
**税引後の再評価（土曜設計書 提案3/9）:** SELL / SWAP アクションには自動で
「売却額（税引前）/ 手取り（税引後）/ 摩擦 / 乗り換え損益分岐 / 口座区分」の表と、
入金で代替できるかの試算が付く。含み益がある売却は 20.315% のハンデを背負うため、
これを添えない提案は構造的に過剰になる。算出できない場合は「算出不可」と明示され、
**税引前の金額が手取りとして扱われることはない**。

### reconcile -- 三点照合（模型 / 実在 / 意図）(土曜設計書 提案1)

証券口座の実残高と模型を突き合わせ、幽霊・未記録・孤児ポジションを検出する。
土曜レポートの第1セクションであり、**照合が通る前に分析してはならない**。

```bash
python3 .../run_portfolio.py reconcile
python3 .../run_portfolio.py reconcile --no-prices          # 価格取得を省く（高速）
python3 .../run_portfolio.py reconcile --rakuten-csv <PATH> # CSV を明示
python3 .../run_portfolio.py reconcile --apply-corporate-actions
python3 .../run_portfolio.py reconcile --json
```
...
| 状態 | 意味 |
|:---|:---|
| **幽霊ポジション** | 模型にあるが口座に不在。存在しない資産のリスクを計算している |
| **未記録ポジション** | 口座にあるが模型に無い。取り込み漏れ |
| **数量差分** | 分割/併合・端株・記録漏れ・原因不明に分類。分割のみ自動補正できる |
| **孤児ポジション** | thesis も政策も無い保有。損切りも利確も判断基準が存在しない |

**⚠️ 独立検証について:** `config/weekly_holdings.yaml` は楽天CSVの取り込み出力であり、
同時に週次レポートの入力でもある。取り込み後に同じCSVと照合すると必ず一致するため、
判定は `ok` ではなく **`circular`（独立検証なし）** になる。

**独立照合にするには、新しいCSVを落としたら取り込む前に reconcile を実行する**
（新CSV × 前回取り込み時点の模型 の突合になる）。

moomoo は US LV3 で米国株のみ見えるため、scope 外（日本株・投信）は
「幽霊」ではなく **`unverified`（残高不明）** に落ちる。
