...
#: 予測ホライズン(営業日)
#:
#: ``quarter`` (約3ヶ月) は後から足した。**「短期の次が半年」では判断の解像度が粗すぎ、
#: 四半期決算1〜2回ぶんという最も意思決定に使う窓が抜けていた**ため。
#: 既存の short/mid/long は互換のため残す(キー名を変えると formatter が壊れる)。
#: ⚠️ タプル第1要素の短縮ラベルは `HORIZON_LABELS` と必ず一致させること。
#: 以前は mid が「中期」、long が「long」と食い違っていた(未使用のため気づかれない)。
HORIZONS = {
    "short": ("短期", 21),        # 約1ヶ月
    "quarter": ("中期", 63),      # 約3ヶ月 ← 決算1〜2回ぶん
    "mid": ("長期", 126),         # 約6ヶ月
    "long": ("超長期", 252 * 3),  # 約3年
}
...
#: レポートに出す順序。dict の挿入順に依存させない(将来の追加で崩れる)。
HORIZON_ORDER = ("short", "quarter", "mid", "long")
