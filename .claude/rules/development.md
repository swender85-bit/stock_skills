...
- moomoo(OpenD) は **`futu-api` の導入が必須**（requirements.txt に固定済み）。未導入だと設定が全て正しくても13カテゴリ全部が静かに落ちる（2026-08-01 の「moomoo無効」警告の実際の原因）。**moomoo はニュース・マクロ材料専用**で、残高照合には使わない（口座に資金が無く必ず available=False になるため。`MOOMOO_AS_BALANCE_SOURCE=on` で戻せる）。
...
`.claude/agents/` に役割別サブエージェントを定義。コスト最適化のためモデルを段階配置する。

| エージェント | model | 役割 |
|:---|:---|:---|
| `screener` | haiku | 機械的なスクリーニング・データ収集の実行。判断はしない |
| `analyst` | sonnet | 数値と過去コンテキストの解釈・レポート整形 |
| `judge` | inherit（親=最上位） | 投資判断の最終意思決定＋Plan-Check Review相当の検証 |

**注意（コンテキスト衛生）:**
- サブエージェントは**独立したコンテキスト**で起動し、`CLAUDE.md` を自動では読み込まない。前提が必要なら明示的に渡すか、エージェント定義本文に書く。
- ビルトインの `Explore` / `Plan` エージェントも同様に `CLAUDE.md` の投資前提・保護ルールを引き継がない。破壊的操作や保有前提が絡む指示を渡すときは、必要な制約を明記する。
- 会話が長くなるとコンテキストは要約される（チェックポイント）。要約後も作業は継続できるが、**要約に載った「事実」は書かれた時点のもの**。ファイル/関数/フラグ名は使う前に実在を確認する。
...
- `python3 -m pytest tests/ -q` で全テスト実行（約4971テスト、~20秒）
