...
5. 機会        — 制約内での保有の立ち位置（銘柄別・過熱横断）
                  ※3と4を通過した後にのみ提示
5.9 これから   — 短期(1ヶ月)/中期(3ヶ月)/長期(半年以降)の見通し
                  ※**静穏週でも畳まない。** 「今週は何も起きなかった」と
                    「これからどこへ向かうか分からない」は別のこと
...
### 🔴 `.bat` は ASCII のみ（2026-08-06 に判明した真因）

**週次深掘りは一度も自動実行できていなかった。** `WeeklyDeep` の `LastRunTime` は
`1999/11/30`（＝一度も成功なし）のままだった。

原因はスケジューラ設定ではなく **`.bat` の日本語コメント**だった。

> **cmd.exe はバッチファイルを UTF-8 ではなく OEM コードページ（cp932）で解釈する。**
> ファイル冒頭の `chcp 65001` はコンソール出力の設定で、バッチ自身の解釈には効かない。
> マルチバイトのコメントが行構造を壊し、**コメントのつもりの文字列が実行された。**

実際に起きたこと: REM 行にバッククォートで囲んだ claude CLI の記述があり、
それが本物のコマンドとして起動され、CLI が `error: unknown option` を返して
**exit 1 で即死。ログを1行も書かずに死ぬ**ため `output/weekly_deep.log` は
存在すらしなかった。「タスクは起動しているのに何も起きない」状態。

対策（`tests/test_bat_encoding.py` が強制）:

| 規則 | 理由 |
|:---|:---|
| `.bat` に非ASCII文字を入れない | cp932 解釈で行構造が壊れる |
| `.bat` にバッククォートを入れない | コメントから漏れた瞬間にコマンド実行になる |
| REM 行を `^` で終えない | REM の中でも行継続として働き次行を飲み込む |
| ログ書き込みを python 呼び出しより前に置く | 早期死亡の痕跡を必ず残す |

**バッチのコメントは英語で書くこと。** 日本語の説明は `.py` 側か rules に置く。
...
| 5.9 これから | 短期(1ヶ月)/中期(3ヶ月)/長期(半年以降)。各ホライズンで **①レンジ ②何がそれを決めるか ③外れる条件** | `projection`, `vol_calibration`, `regime`, `constituents`, `forward_horizon`, `leverage_sleeve`, `scenarios` |
| 6 事前決定 | 翌週の条件付き政策（登録コマンド付き） | 本文 + `forward` |
| 7 監査 | 累積差分（4週/13週）・執行/模型の健全性 | `cumulative_diff` |
| 8 前提と限界 | 予測前提・税務免責・系譜サマリ | 本文 |

**節 5.9 の制約**: 確率を書かない（書けるのはレンジと傾きと効く出来事まで）。
3つの時間軸で結論が同じなら分析していない。レバレッジの非対称性（往復すると戻らない）
を必ず具体額で示す。この節は本文依存（`needs_body`）で、ここまでの全節を踏まえて書く。

補助材料: 推移予測（短期1ヶ月/中期3ヶ月/長期6ヶ月/超長期3年のレンジ + 置いた前提）、
